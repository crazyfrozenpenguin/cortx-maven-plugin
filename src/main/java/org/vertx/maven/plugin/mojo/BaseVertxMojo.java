...
public abstract class BaseVertxMojo extends AbstractMojo {
	 ...
	 */
	@Parameter(property = "moduleName", defaultValue = "org.cortx~cortx-maven-plugin")
	...
	protected PlatformManager createPlatformManager() {
		...
		if (cluster || ha) {
			...
			if (ha) {
				final int quorumSize = quorum == null ? 0 : Integer.valueOf(quorum);
	...
	protected void setVertxMods() throws Exception {
		final String vertxMods = System.getenv("VERTX_MODS");
		if (vertxMods != null) {
	...
	private void addURLs(final List<URL> urls, final String dirName) throws IOException {
	...
	private static class LoadFirstClassLoader extends URLClassLoader {
		...
		private LoadFirstClassLoader(final URL[] urls, final ClassLoader parent) {
			super(urls, parent);
			this.parent = parent;
		}
