...
/**
 * @description Starts (non-blocking) the cortx embedded verticle or given
 *              vert.x module in maven space.
 */
@Mojo(name = "start", requiresProject = true, threadSafe = false, requiresDependencyResolution = COMPILE_PLUS_RUNTIME)
					...
					if (moduleName.equals("org.cortx~cortx-maven-plugin")) {
						...
						try {
							...
							pm.deployVerticle("org/cortx/maven/plugin/module/cortx.js", config, ((URLClassLoader) Thread
									.currentThread().getContextClassLoader()).getURLs(), 1, null,
